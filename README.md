...
docker build -t frame-controller:1.4.0 .
```
...
```bash
docker build --no-cache -t frame-controller:1.4.0 .
```
...
```bash
docker run --rm --user 3019:3019 --entrypoint python frame-controller:1.4.0 -c "import controller, webui; print('Application readable')"
```
